...
Clerk also needs **Organizations** enabled, and its JWT issuer domain must match
`convex/auth.config.js` so Convex trusts Clerk-issued tokens.
...
- Marking accuracy is bounded by the vision model. Llama 4 Scout is fast enough to run on
  every stroke, but very messy handwriting or long multi-step proofs can still be misread.
  Swapping `recognize-math` to a stronger vision model is the highest-leverage improvement.
- Marker placement maps model verdicts to stroke-line anchors **by index**, so if the model
  reports a different number of lines than the geometry detects, later markers can shift.
- Base64 image storage inflates document size for large uploads.
- SVG rendering will degrade on drawings with many thousands of strokes.
